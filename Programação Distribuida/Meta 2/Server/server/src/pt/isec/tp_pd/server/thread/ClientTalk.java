...
public class ClientTalk extends Thread {
    ...
    public ClientTalk(Socket cliSocket, ServerData serverData, ServerRequest serverRequest,
                      ...
                      ObjectOutputStream objOutputStream,ObjectInputStream objInputStream, rmiService a) {
                        ...
                        if(serverRequest.getClientDetailed().getCLM().getUsername().equals("admin")){
                            ...
                            for (Espetaculo e : espetaculos){
                                ...
                                try {
                                    ...
                                    a.notifyListeners(NotifyCode.LOSTCONNECTIONTCP,"Lost Connection Client: " + cliSocket.getInetAddress().getHostAddress());
                                    a.notifyListeners(NotifyCode.LOGOUTCLIENT,"Partial Logout Client: " + cliSocket.getInetAddress().getHostAddress());
                                    System.out.println("O cliente 🤡 " + cliSocket.getInetAddress().getHostAddress() + " desconectou-se. 👋 Bye bye, see you later!");
                                    try {
