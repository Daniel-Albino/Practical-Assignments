...
import pt.isec.tp_pd.sync.SendSignalUDP;
...
public class InitialUI extends BorderPane {
    ...
    String ip, port;
    public InitialUI(ModelManager model) {
        this.model=model;
        createViews();
        registerHandlers();
        update();
    ...
    private void registerHandlers() {
        ...
        btnLogin.setOnAction( event -> {
            ...
            if(!ip.isEmpty() && !port.isEmpty()) {
                SendSignalUDP a = new SendSignalUDP(model,PagesEnum.LOGIN);
        ...
        btnSignup.setOnAction( event -> {
            ...
            port = listeningPort.getText().trim();
            if(!ip.isEmpty() && !port.isEmpty()) {
                SendSignalUDP a = new SendSignalUDP(model,PagesEnum.SIGNUP);
                model.setAttributes(ipAddress.getText(), listeningPort.getText());
                ipAddress.clear();
                listeningPort.clear();
                model.ConnectionPage();
    ...
    private void update() {
        if (model.getState() != TicketPdState.INITIALSTATE) {
